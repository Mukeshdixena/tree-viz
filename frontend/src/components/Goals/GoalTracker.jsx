...
import LeafNodeDetails from '../LeafNodeDetails';
...
import '../../App.css'; // Maintaining original styles for now
...
function GoalTracker() {
    ...
    const syncWithBackend = async (newData) => {
        try {
            await api.post('/trees/sync', newData);
        } catch (err) {
    ...
    if (loading) {
        return (
            <div className="loading-screen" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Loader2 className="animate-spin" size={48} />
                <span style={{ marginLeft: '1rem' }}>Loading Roadmaps...</span>
            </div>
        );
    }

    return (
        <div className={`App ${isSidebarOpen ? 'sidebar-open' : ''} ${isEditorOpen || isDetailsOpen ? 'editor-open' : ''}`} style={{ height: '100%', width: '100%' }}>
            <Sidebar
                data={data}
